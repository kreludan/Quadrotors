...
int last_heartbeat_v=-1;
long safety_time_prev_v;
long safety_time_curr_v;
float time_since_heartbeat_v=0;
int last_version=-1;
int Thrust = 0;
int neutral_thrust = 1300; // prev 1450
int pwm;
...
#define PWM_MAX 1900    // Changed for Milestone 2/3 week 7
#define frequency 25000000.0
#define LED0 0x6			
...
void pid_update() {
  ...
  local_p = *position;
  //float desired_yaw = 125.0*(shared_memory->yaw - 128.0)/112.0;
  float desired_yaw_val = 0;
  float actual_yaw = local_p.yaw;
  float yaw_p = 200;
  float yaw_diff = actual_yaw-desired_yaw_val;
  float desired_yaw = yaw_p*yaw_diff;

printf("%f, %f, %f\n\r", actual_yaw, yaw_diff, desired_yaw);

  float desired_pitch = 6.0*(shared_memory->pitch - 128.0)/112.0;  
  float desired_roll = 6.0*(shared_memory->roll - 128.0)/112.0;
  ...
  float Py = 2;
  float yaw_error = desired_yaw-imu_data[2];  
  float p_yaw = yaw_error*Py;
  ...
  */
  ...
  float right_PDr = roll_error*Pr + right_Dr;
  float left_PDr = - roll_error*Pr + left_Dr;
  
  float right_PIDr = - roll_error*Pr + right_Dr - Roll_I_term;
  float left_PIDr = roll_error*Pr + left_Dr + Roll_I_term;
  ...
  float right_PID = error*P + right_D + Pitch_I_term;
  float left_PID = -error*P + left_D - Pitch_I_term;
...
*/
  ...
  float total_0 = Thrust + right_PID + right_PIDr + p_yaw;
  float total_1 = Thrust + right_PID + left_PIDr - p_yaw;
  float total_2 = Thrust + left_PID + left_PIDr + p_yaw;
  float total_3 = Thrust + left_PID + right_PIDr - p_yaw;
  total_0 = clamp(total_0);
  total_1 = clamp(total_1);
  total_2 = clamp(total_2);
  total_3 = clamp(total_3);
  
  set_PWM(0, total_0);
  set_PWM(1, total_1);
  set_PWM(2, total_2);
  set_PWM(3, total_3);
  //printf("%f, %f, %f, %f, %f, %f\n\r", desired_yaw, imu_data[2], total_0, total_1, total_2, total_3);
  //printf("%f, %f\n\r", desired_yaw, imu_data[2]);
  // printf("%10.5f, %10.5f \n\r", desired_roll, Roll);
}
...
// xyz gyro values read from imu_data[0..2]
 ...
 printf("FAILING DUE TO GYRO LIMITS");
  ...
  // Vive checks
  local_p = *position;
  float curr_x = local_p.x;
  float curr_y = local_p.y;

  // printf("curr x,y: %5.5f,%5.5f | desired x,y: %5.5f,%5.5f\n\r", curr_x,curr_y, Desired_x, Desired_y);


  if(curr_x - Desired_x >= 1000 || curr_x - Desired_x <= -1000){
    printf("FAILING DUE TO X POSITION %5.5f WHEN DESIRED IS %5.5f", curr_x, Desired_x);
    run_program = 0;
  }

  if(curr_y - Desired_y >= 1000 || curr_y - Desired_y <= -1000){
    printf("FAILING DUE TO Y POSITION %5.5f WHEN DESIRED IS %5.5f", curr_y, Desired_y);
    run_program = 0;    
  }  

  timespec_get(&te,TIME_UTC);
  safety_time_curr_v=te.tv_nsec;
  //compute time since last execution
  imu_diff=safety_time_curr_v-safety_time_prev_v;  
  int curr_heartbeat_v = local_p.version;
  ...
  safety_time_prev_v=safety_time_curr_v;
  

  
  if(curr_heartbeat_v != last_heartbeat_v){
    time_since_heartbeat_v = 0;
  }
  
//  printf("curr: %d, last: %d, time: %d, curr x,y: %f,%f --- desired x,y: %f,%f\n\r", curr_heartbeat_v, last_heartbeat_v, time_since_heartbeat_v, curr_x,curr_y, Desired_x, Desired_y);
  
  if(curr_heartbeat_v == last_heartbeat_v){
  
  time_since_heartbeat_v += imu_diff;
  }
  
  printf("Time since: %10.5f", time_since_heartbeat_v);

  if(time_since_heartbeat_v >= 0.6 && last_heartbeat_v == curr_heartbeat_v){
  printf("FAILING DUE TO NO HEARTBEAT SIGNAL FROM VIVE, %10.5f", time_since_heartbeat_v);
  
  run_program=0;
  }
  
  last_heartbeat_v = curr_heartbeat_v;  
...
int main (int argc, char *argv[])
{
    ...
    while(run_program==1)
    {
      ...
      local_p = *position;
      /*
      Can now use vive sensor values e.g.
      local_p.version
      local_p.x
      local_p.y
      local_p.z
      ...
      */
      ...
      previous_Roll = Roll;
      Keyboard keyboard=*shared_memory;
      Thrust = neutral_thrust + 0.85*shared_memory->thrust;
      get_joystick();
